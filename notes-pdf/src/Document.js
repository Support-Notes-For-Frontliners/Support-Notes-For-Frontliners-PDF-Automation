...
function MyDocument() {
  ...
  function generateBoolean(data){
    ...
    for(i = 0; i < keyList.length; i++){
      ...
      console.log("Facility Name: " + facilityName + " Facility Status: " + facilityStatus + " entryFacility:" + entryFacility )
        ...
        {Object.entries(firebaseData).map(data => (
          generateBoolean(data) ? 
            <View style={sectionStyle(data[1]["note"].length)} wrap={false}>
              ...
              <Text>{` `}</Text>
              <Text>{data[1]["note"].replace(/(\r\n|\n|\r)/gm, "").replace("Dear Nursing Home worker,","").replace('Dear Health Care Worker,', '').replace("Dear Healthcare worker,", "")}</Text>
              <Text style={{textAlign: 'right'}}>-From {data[1]["sender"]}</Text>
            </View> : null
         ))}
          </Page>
      </Document>
